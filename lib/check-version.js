...
module.exports = done => {
    ...
    if (packageConfig.engines && !semver.satisfies(process.version, packageConfig.engines.node)) {
        return console.log(chalk.red(
            ' You must upgrade node to >=' + packageConfig.engines.node + '.x to use vue-cli postline-cli'
        ))
    }

    request({
        url: 'https://registry.npmjs.org/postline-cli',
        timeout: 1000
    }, (err, res, body) => {
        if (!err && res.statusCode === 200) {
            ...
            if (semver.lt(localVersion, latestVersion)) {
                console.log(chalk.yellow('  A newer version of postline-cli is available.'))
                console.log()
